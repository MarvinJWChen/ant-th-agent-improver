...
def reset_configs(baseline: str = "v1") -> dict[str, int]:
    """Return the config registry to just the baseline, active.

    Candidates are regenerated by the patch step, so dropping them is what makes
    a reset a genuinely fresh start rather than a half-used one.
    """
    with sqlite3.connect(CONFIGS_DB_PATH()) as c:
        removed = c.execute(
            "DELETE FROM agent_configs WHERE version != ?", (baseline,)
        ).rowcount
        c.execute(
            "UPDATE agent_configs SET status = 'active' WHERE version = ?", (baseline,)
        )
    return {"candidates_removed": max(removed, 0)}


def CONFIGS_DB_PATH():
    return paths.CONFIGS_DB
